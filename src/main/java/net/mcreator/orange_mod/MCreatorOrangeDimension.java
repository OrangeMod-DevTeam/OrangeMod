...
public class MCreatorOrangeDimension extends Elementsorange_mod.ModElement {
	public static int DIMID = 3;
	public static final boolean NETHER_TYPE = true;
	@GameRegistry.ObjectHolder("orange_mod:orangedimension")
	public static final Item block = null;
	@GameRegistry.ObjectHolder("orange_mod:orangedimension_portal")
	public static final BlockCustomPortal portal = null;
	public static DimensionType dtype;

	public MCreatorOrangeDimension(Elementsorange_mod instance) {
		super(instance, 15);
	}
	...
	public void initElements() {
		...
		elements.items.add(() -> new ModTrigger().setUnlocalizedName("orangedimension").setRegistryName("orangedimension"));
	}
	...
	public void preInit(FMLPreInitializationEvent event) {
		if (DimensionManager.isDimensionRegistered(DIMID)) {
			...
			System.err.println("Dimension ID for dimension orangedimension is already registered. Falling back to ID: " + DIMID);
		}
		dtype = DimensionType.register("orangedimension", "_orangedimension", DIMID, WorldProviderMod.class, true);
	...
	public void registerModels(ModelRegistryEvent event) {
		ModelLoader.setCustomModelResourceLocation(block, 0, new ModelResourceLocation("orange_mod:orangedimension", "inventory"));
	}
				...
				for (int j7 = -1; j7 <= 1; ++j7) {
					for (int l7 = 1; l7 < 3; ++l7) {
						for (int k8 = -1; k8 < 3; ++k8) {
							...
							boolean flag = k8 < 0;
							this.world.setBlockState(new BlockPos(k9, k10, k11), flag ? MCreatorOrangeObsidian.block.getDefaultState().getBlock()
			...
			for (int i8 = 0; i8 < 4; ++i8) {
				for (int l8 = 0; l8 < 4; ++l8) {
					for (int l9 = -1; l9 < 4; ++l9) {
						...
						this.world.setBlockState(new BlockPos(l10, l11, k12), flag1 ? MCreatorOrangeObsidian.block.getDefaultState().getBlock()
				...
				for (int j1 = -2; j1 <= 2; ++j1) {
					for (int k1 = -2; k1 <= 2; ++k1) {
						for (int l1 = -1; l1 < 3; ++l1) {
							...
							int k2 = k + k1 * 0 - j1 * 1;
							boolean flag = l1 < 0;
							this.world.setBlockState(new BlockPos(i2, j2, k2), flag ? MCreatorOrangeObsidian.block.getDefaultState().getBlock()
									.getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
	...
	public static class BlockCustomPortal extends BlockPortal {
		public BlockCustomPortal() {
			setHardness(-1.0F);
			setUnlocalizedName("orangedimension_portal");
			setRegistryName("orangedimension_portal");
			setLightLevel(0F);
		}
		...
		public static class Size {
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, EnumFacing p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.isEmptyBlock(this.world.getBlockState(blockpos).getBlock())
							|| this.world.getBlockState(blockpos.down()).getBlock() != MCreatorOrangeObsidian.block.getDefaultState().getBlock()) {
						break;
					}
				}
				Block block = this.world.getBlockState(p_180120_1_.offset(p_180120_2_, i)).getBlock();
				return block == MCreatorOrangeObsidian.block.getDefaultState().getBlock() ? i : 0;
			}
			...
			protected int calculatePortalHeight() {
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (block != MCreatorOrangeObsidian.block.getDefaultState().getBlock()) {
								break label56;
							}
				...
				for (int j = 0; j < this.width; ++j) {
					if (this.world.getBlockState(this.bottomLeft.offset(this.rightDir, j).up(this.height)).getBlock() != MCreatorOrangeObsidian.block
	...
	public static class ChunkProviderModded implements IChunkGenerator {
		private static final IBlockState STONE = MCreatorOrangeObsidian.block.getDefaultState();
		private static final IBlockState STONE2 = Blocks.NETHERRACK.getDefaultState();
		private static final IBlockState STONE3 = MCreatorOrangeObsidian.block.getDefaultState();
		private static final IBlockState FLUID = MCreatorOrangeWater.block.getDefaultState();
